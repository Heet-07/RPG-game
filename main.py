...
class Game:
    ...
    def update(self):
        ...
        if not self.enemy.alive and self.state == "playing":
            # Mark level as completed
            self.level_completed[self.level_number] = True

            # Always go to "level_complete", even if last level
    ...
    """def draw_game_complete(self):
        ...
        draw_text(self.screen, "🎉 Congratulations! 🎉", self.font, GREEN,
                SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 60, center=True)
        draw_text(self.screen, "You’ve completed all levels!", self.font, WHITE,
                SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 20, center=True)
        draw_text(self.screen, "Press R to Restart", self.font, WHITE,
